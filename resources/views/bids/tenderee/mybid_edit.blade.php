...
<div class="warpper-main">
	<form id="bids-form" role="form" action="/tenderee/my/{{$bid->id}}/edit" method="POST">
					...
					<div class="route-page-bids-1">
							...
							<div>
								<select name="project_id" class="form-control" id="project-name" required>
                                    ...
                                    <?php $myCount='';  ?>
                                        ...
                                        @if($project['pid']==$bid->pid)
                                            <option id="{{ $project['pid'] }}" value="{{ $project['pid'] }}" selected='selected'>{{ $project['name'] }}</option>
                                            <?php $myCount=$project['pid']; ?>
                                        @else
						...
						<div class="form-for-2">
							<div class="form-group" select-connect>
								...
								<input name="contact_name" type="text" class="form-control" value={{$bid->contact_name}} required />
							</div>
							<div class="form-group" select-connect>
								<label for="exampleInputPassword1" >联系人电话</label>
								<input name="contact_phone" type="text" class="form-control" value={{$bid->contact_phone}} number="true" minlength='11' maxlength="11" required />
							</div>
					...
					<div class="route-page-bids-2">
						...
						<div>
							...
							<div class="form-group select-time">
								...
								<input name="bid_deadline" type="text" class="form-control time-select" readonly="true" value="{{$bid->bid_deadline}}" required>
							</div>
							<div class="form-group select-time">
								<label for="exampleInputPassword1">开标时间</label>
								<input name="bod" type="text" class="form-control time-select" readonly="true" value="{{$bid->bod}}" required>
							</div>
							{{-- <div class="form-group select-time">
								<label for="exampleInputPassword1">供货时间</label>
								<input name="affords_time" type="text" class="form-control time-select" readonly="true" value="{{$bid->delivery_day}}" required>
							</div>
							<div class="form-group select-time">
                            ...
                            <div class="form-group select-time">
								...
								<input name="delivery_day" type="text" class="form-control" value="{{$bid->delivery_day}}" required>
							</div>
							<div style="clear:both;"></div>
						</div>
								...
								@foreach ($q_request as $r)
                                  {{-- @if(!empty($r)) --}}
                                    @if($r == '含17%增值税发票')
                                     ...
                                     @else
                                        <?php $tmp_qid=qr_to_id($r);?>
                                        @if(in_array($tmp_qid,$bid->quote_request))
                                        <label>
                                            <input name="q_request[]" type="checkbox" value="{{ qr_to_id($r) }}" data-name="{{ $r }}" data-labelauty="{{ $r }}" checked="checked"/>
                                        </label>
                                        @else
                                        <label>
						...
						<div>
							<div class="form-group" style="height:auto;">
								...
								<textarea style="display:block;width:100%;height:150px;" class="settlement" name="settlement" type="text" class="form-control" placeholder="" required>{{$bid->settlement}}</textarea>
								<div style="clear:both;"></div>
							</div>
						...
						<table class="table table-bordered" id="bids-table">
							...
							<tbody>
                                {{-- @foreach ($bid->quote_list as $key=>$item)
									<tr>
										<td>{{$key+1}}</td>
										<td><select name="cname[]" class="frist-select" required>{{get_cname_by_id($item[0])}}</select></td>
										<td>{{get_size_by_id($item[1])}}</td>
										<td>{{get_material_by_id($item[2])}}</td>
                                        <td>{{get_material_by_id($item[3])}}</td>
                                        <td id="all-count" rowspan ="1">0</td>
                                        <td><input name="mark[]" type="text" /></td>
                                        <td><button type="button" class="btn btn-danger bids-dele-btn" disabled="true">删除</button></td>
								    <tr>
                                @endforeach --}}
                                <?php $count=0; ?>
                                 @foreach ($bid->quote_list as $key=>$item)
									...
									<td>
										<select name="cname[]" class="frist-select" required>
                                            @foreach($cats[0]['child'] as $ck=>$cname)
                                                @if($cname['id']==$item[0])
                                                <?php $tmp_a_gg=$cats[0]['child'][$ck]['child'][0]['child'];?>
                                                <?php $tmp_a_cz=$cats[0]['child'][$ck]['child'][1]['child'];?>
                                                <option value="{{$cname['id']}}" selected='selected'>{{$cname['name']}}</option>
                                                @else
									...
									<td>
										...
										<select name="size[]" class="" required>
                                             @foreach($tmp_a_gg as $ggk=>$ggv)
                                                @if($ggv['id']==$item[1])
                                                    <option value="{{$ggv['id']}}" selected='selected'>{{$ggv['name']}}</option>
                                                @else
                                                    <option value="{{$ggv['id']}}">{{$ggv['name']}}</option>
                                                @endif
                                             @endforeach
										</select>
									</td>
									<td>
										<select name="material[]" class="three-select" required>
                                                @foreach($tmp_a_cz as $czk=>$czv)
                                                @if($czv['id']==$item[2])
                                                    <option value="{{$czv['id']}}" selected='selected'>{{$czv['name']}}</option>
                                                @else
                                                    <option value="{{$czv['id']}}">{{$czv['name']}}</option>
                                                @endif
                                                @endforeach
										</select>
									</td>
									<td>
										<input name="amount[]" type="number" value={{$item[3]}} required />
									</td>
                                    @if($key==0)
									<td id="all-count" rowspan ="{{count($bid->quote_list)}}">{{$bid->batch_amount}}</td>
                                    @endif
									<td>
										<input name="mark[]" type="text" value="{{$item[4]}}" />
									</td>
									<td>
										@if($count==0)
			...
			<div id="collapseThree" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingThree">
				<div class="panel-body">
					...
					<div class="form-group">
					    ...
					    <label class="pub-input">
                            @if(empty($bid->companys))
					    	<input type="radio" name="bid_to" value="all" data-labelauty="公开招标"  onclick="$('.select-company').hide()" checked='checked' />
                            @else
                            <input type="radio" name="bid_to" value="all" data-labelauty="公开招标"  onclick="$('.select-company').hide()" />
                            @endif
					    </label>
					    <label class="pre-input">
                        @if(empty($bid->companys))
					    	<input type="radio" name="bid_to" value="some" data-labelauty="定向招标" onclick="$('.select-company').show()" />
                        @else
                            <input type="radio" name="bid_to" value="some" data-labelauty="定向招标" onclick="$('.select-company').show()" checked='checked' />
                        @endif
					    </label>
					</div>
					<div class="form-group select-company">
						...
						@foreach ($companys as $cid=>$company)
                            {{-- @if(!empty($bid->companys)) --}}
                                @if(!empty($bid->companys) && in_array($cid,$bid->companys))
                                    <option value={{$cid}} selected='selected'>{{ $company }}</option>
                                @else
				...
				<button onclick="toSubmit()" type="button" id="bids-route-btn" class="btn btn-success set-width-8" style="max-width:500px;display:block;margin:30px auto 10px auto;">更新招标</button>
				<br /><br />
			</div>
		</div>
	</div>
...
<script>
	...
	$(document).ready(function(){
		...
		var init_bid = {!! json_encode($init_bid) !!};
		//console.log(init_bid);
		//console.log(projectData);

		_initbid(init_bid, $("#project-name").val());
		...
		function _initbid(datas, string){
			for(key in datas){
				//console.log(key ,string)
				if(key === string){
					//设置省份
		...
		(function(){
            ...
            var ql = {!! json_encode($bid->quote_list) !!};
            var companys = {!! json_encode($bid->companys) !!};
            ...
            var ql = {!! json_encode($bid->quote_list) !!};
            var cids = {!! json_encode($bid->companys) !!};
